...
func PrefixQuery(indexName string, iQuery mbase.MangoQuery) (*bleve.SearchResult, error) {
	requestedIndex := mbase.INDEX_LIST[indexName]

	dateQuery := bleve.NewDateRangeQuery(iQuery.Query.StartTime, iQuery.Query.EndTime)
	prefixQuery := bleve.NewPrefixQuery(iQuery.Query.Term)
	query := bleve.NewConjunctionQuery(dateQuery, prefixQuery)
